...
public class CalendarDB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String sql = "CREATE TABLE mood (" + "year int(11), " + "month int(11), " + "day int(11), "
                + "mood_code varchar(64), " + "updated_time timestamp, " + "description varchar(255) DEFAULT '');";
        db.execSQL(sql);
    ...
    public Map<String, CalendarCell> getMood(int year, int month) {
        SQLiteDatabase db = getWritableDatabase();
        Cursor cursor = db.query("mood", new String[] { "mood_code", "day", "updated_time", "description" },
                "year=? AND month=?", new String[] { "" + year, "" + month }, null, null, null);
    ...
    public int getMinYear() {
        SQLiteDatabase db = getWritableDatabase();
        Cursor cursor = db.query("mood", new String[] { "min(year)" }, null, null, null, null, null);
        try {
            while (cursor.moveToNext()) {
