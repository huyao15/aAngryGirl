...
public class CalendarData {
	...
	public CalendarCell getCell(int pos) {
		...
		if (null == cell) {
			cell = new CalendarCell(year, thatMonth, thatDayOfMonth,
					String.valueOf(thatDayOfMonth), cellType);
		}

		int today = Calendar.getInstance().get(Calendar.DAY_OF_YEAR);
		int thatDay = cal.get(Calendar.DAY_OF_YEAR);
		if (thatDay == today
				&& thatYear == Calendar.getInstance().get(Calendar.YEAR)) {
			cell.setCanChange(true);
			cell.setToday(true);
		} else if (thatDay < today && cell.getMood() == Mood.UNKNOWN) {
			// 所有之前没有改过的都可以改一次
			cell.setCanChange(true);
		}
